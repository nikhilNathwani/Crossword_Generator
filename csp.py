  ...
  def solveCSP(self):
  	while(len(self.needWords)!=0):
	  	...
	  	if node.word==[" " for _ in xrange(node.len)]:
	  		...
	  		node.word[index]= 'a'
		query= ""
		for letter in node.word:
			if letter==" ": query+="?"
			else:  query+=letter
		possibleWords= scrape.getPossibleWords(query)
		if possibleWords==[]: print "oh noooo"
		else: 
			print possibleWords
...
#GUI STUFF
    ...
    gameboard = gui.GameBoard(leftFrame, cross, cross.solution, 6*80)
    clues= gui.Clues(rightFrame, cross.acrossClues, cross.downClues, 6*6)
